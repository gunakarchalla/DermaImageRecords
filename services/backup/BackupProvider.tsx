...
import { backupToDriveAsync, type BackupProgress } from "./backupService";
import {
...
export function BackupProvider({ children }: { children: ReactNode }) {
    ...
    const runBackup = useCallback(async (): Promise<void> => {
        ...
        try {
            ...
            persist({ ...current, driveFileId: fileId, lastBackupAt: Date.now() });
        } catch (error) {
    ...
    const backupNow = useCallback(() => runBackup(), [runBackup]);

    // Automatic backup: on app foreground (and once on ready), back up if the chosen
    // period has elapsed since the last success. Won't run while the app is closed.
    const maybeAutoBackup = useCallback(() => {
        const s = settingsRef.current;
        if (!signedInRef.current) return;
        if (s.mode !== "automatic") return;
        if (runningRef.current) return;
        const intervalMs = periodDays(s.periodKey, s.customDays) * DAY_MS;
        const due = s.lastBackupAt == null || Date.now() - s.lastBackupAt >= intervalMs;
        if (!due) return;
        // Auto-runs are silent; failures surface via lastError, not an alert.
